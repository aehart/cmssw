...
bool VMStubsTEMemory::addVMStub(VMStubTE vmstub, int longvmbin, int bin) {
  ...
  if (settings_.combined()) {
    ...
    if (negdisk)
      bin += 4;
    assert(longvmbin + bin < (int)stubsbinnedvm_.size());
    if (stubsbinnedvm_[longvmbin + bin].size() < N_VMSTUBSMAX) {
      stubsbinnedvm_[longvmbin + bin].push_back(vmstub);
  ...
  if (overlap_) {
    if (disk_ == 1) {
      ...
      if (negdisk)
        bin += 4;
      if (stubsbinnedvm_[longvmbin + bin].size() >= settings_.maxStubsPerBin())
        return false;
      stubsbinnedvm_[longvmbin + bin].push_back(vmstub);
      if (settings_.debugTracklet())
